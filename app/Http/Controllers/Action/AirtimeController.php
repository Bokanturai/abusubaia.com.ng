...
class AirtimeController extends Controller
{
    ...
    public function buyAirtime(Request $request)
    {
        ...
        // Map network names to Abu subaia verification API codes
        $networkCodes = [
            'airtel' => '100',
            'mtn'    => '101',
            'glo'    => '102',
        ...
        // 5. Call Abu subaia verification Airtime API
        try {
            ...
            Log::error('Abu subaia verification API Connection Error: ' . $e->getMessage());
            return redirect()->back()->with('error', 'Could not connect to airtime provider. Please try again later.');
        }

        // 6. Process Response
        $data = $response->json();
        Log::info('Abu subaia verification API Response', ['response' => $data]);
        ...
        Log::error('Abu subaia verification API Response Error', ['response' => $data]);
