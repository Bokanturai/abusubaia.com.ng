...
class DataController extends Controller
{
    ...
    public function buydata(Request $request)
    {
        ...
        // 5. Call Abu subaia verification Data API
        try {
            $response = Http::withHeaders([
                'Authorization' => 'Bearer ' . $this->getApiToken(),
            ...
            Log::error('Abu subaia verification Data API Connection Error: ' . $e->getMessage());
            return redirect()->back()->with('error', 'Could not connect to data provider. Please try again later.');
        }

        // 6. Process Response
        $data = $response->json();
        Log::info('Abu subaia verification Data API Response', ['response' => $data]);
        ...
        Log::error('Abu subaia verification Data API Response Error', ['response' => $data]);
